...
def compute_lbp_histogram(gray):
    ...
    hist = np.bincount(lbp.ravel(), minlength=CFG.LBP_DIM).astype(np.float32)
    hist /= (hist.sum() + 1e-8)
    return hist

def compute_features(patch):
    """Hàm trích xuất HOG+HSV+LBP."""
    ...
    hog_vector = np.asarray(HOG_DESCRIPTOR.compute(gray)).flatten()
    ...
    lbp_vector = compute_lbp_histogram(gray)
    return np.concatenate((hog_vector, color_vector, lbp_vector)).astype(np.float32)
...
if __name__ == "__main__":
    ...
    print("Bắt đầu trích xuất đặc trưng HOG+HSV+LBP...")
